...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        n = torch.arange(max_len)[:, None]
        powrs = torch.pow(10000, -torch.arange(0, embed_dim, 2) / embed_dim)

        # Computing positional values sin/cos
        pe[0, :, 0::2] = torch.sin(n * powrs)
        pe[0, :, 1::2] = torch.cos(n * powrs)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Y = torch.mathmul(Q, K.transpose(2, 3) / self.scale
        ...
        Y = self.dropout(Y)
        Y = torch.mathmul(Y,V)
        output = self.proj(Y.transpose(1, 2).reshape(N, S, H*D))
